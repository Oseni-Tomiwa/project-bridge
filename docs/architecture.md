...
This document describes the active voice-first healthcare-intake slice and provider-neutral boundaries. Intron/Sahara is wired into the product flow through the API; OpenAI and Deepgram remain evaluation adapters. The financial-support implementation is preserved as an inactive prior domain. A three-provider Vocal Money development run exists separately from the product flow; the primary AfriSwitch challenge comparison has not run.
...
- Let vertical implementations contribute action definitions and domain metadata through `DomainModule`. Healthcare intake and the preserved financial-support domain live beside that contract in `packages/domain` to keep the challenge workspace small.
- Represent action confirmation requirements explicitly.
- Hide the active in-memory clinic-intake store behind `ClinicIntakeRepository`; durable persistence remains unselected. The prior `SupportCaseRepository` remains isolated to the preserved financial domain.
...
| `actions`      | action definition/executor contracts, confirmation policy                   | hard-coded vertical workflows                   |
| `benchmark`    | sample/result schemas, runner contract, normalization/WER                   | fabricated or manually altered scores           |
| `domain`       | extension contract, active healthcare intake, preserved financial workflow  | platform-wide provider selection                |
...
5. Deterministic domain rules derive `clinic_intake_request`, preserve reported concern/symptom phrases, and check narrow emergency language.
6. Emergency language stops routine action; otherwise the service asks one needed clarification or creates a non-diagnostic proposal summary.
...
9. The healthcare executor creates one simulated intake through `ClinicIntakeRepository` and returns a `BRG-H-<year>-<opaque>` reference. It does not contact a clinic or book an appointment.
...
- Never convert a reported symptom into a diagnosis, cause, treatment, prescription, or clinical recommendation.
- Keep the emergency-language outcome distinct from normal clarification/action states and reject routine intake after escalation.
...
Clinical review and jurisdictional emergency wording, Sahara model/version identity, browser-format coverage, product-provider fallback policy, OpenAI alias/version and language-hint policy, Deepgram version/language experiment policy, post-prototype interpretation/LLM approach, deployment platform, health-data persistence, authentication, observability, text-to-speech, and detailed confidence calibration are unresolved.
