...
- The completed local `vocal-money-dev-30-v1` run is a secondary development result only. Its primary development aggregate scores 27 samples per provider after holding three uncertain clips; it must not be presented as the primary AfriSwitch benchmark or a final model ranking.
